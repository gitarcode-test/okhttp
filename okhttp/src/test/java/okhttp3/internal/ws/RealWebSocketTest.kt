...
class RealWebSocketTest {
    // NOTE: Fields are named 'client' and 'server' for cognitive simplicity. This differentiation
    // has
    ...
    fun clientCloseWith0Fails() {
        assertFailsWith<IllegalArgumentException> { client.webSocket!!.close(0, null) }
            .also { expected ->
    ...
    fun clientCloseCancelsConnectionAfterTimeout() {
        client.webSocket!!.close(1000, "Hello!")
        taskFaker.runTasks()
        // Note: we don't process server frames so our client 'close' doesn't receive a server
        // 'close'.
    ...
    fun protocolErrorInCloseResponseClosesConnection() {
        ...
        client
            .processNextFrame() // Detects error, disconnects immediately since close already sent.
        client.webSocket!!.finishReader()
        assertThat(client.closed).isTrue()
        client.listener.assertFailure(
            ProtocolException::class.java,
    ...
    fun unexpectedPongsDoNotInterfereWithFailureDetection() {
        ...
        // client to fail at 1500ms when it's time to send ping 3 because pong 2 hasn't been
        // received.
        taskFaker.advanceUntil(ns(1000L))
        taskFaker.advanceUntil(ns(1500L))
        client.listener.assertFailure(
    ...
    private class TestStreams(
        ...
        /**
         * Peeks the number of bytes available for the client to read immediately. This doesn't
         * block so it requires that bytes have already been flushed by the server.
         */
