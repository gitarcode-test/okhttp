...
class Http2ConnectionTest {
    ...
    /**
     * Confirm that we account for discarded data frames. It's possible that data frames are
     * in-flight just prior to us canceling a stream.
     */
    @Test
    ...
    fun receiveGoAwayHttp2() {
        ...
        connection
            .writePingAndAwaitPong() // Ensure the GO_AWAY that resets stream2 has been received.
    ...
    /**
     * When writing a set of headers fails due to an `IOException`, make sure the writer is left in
     * a consistent state so the next writer also gets an `IOException` also instead of something
     * worse (like an [IllegalStateException].
     *
    ...
    fun socketExceptionWhileWritingHeaders() {
        ...
        assertFailsWith<IOException> { connection.newStream(headerEntries("a", longString), false) }
        assertFailsWith<IOException> { connection.newStream(headerEntries("b", longString), false) }
    }
    ...
    fun clientRequestBodyServerResponseBodyRstStream() {
        ...
        val sink = stream.getSink().buffer()
        sink.writeUtf8("abc")
        assertFailsWith<StreamResetException> { sink.close() }
            .also { expected -> assertThat(expected.errorCode).isEqualTo(ErrorCode.NO_ERROR) }
        assertThat(stream.takeHeaders()).isEqualTo(headersOf("a", "android"))
        val source = stream.getSource().buffer()
        assertThat(source.readUtf8(5)).isEqualTo("robot")
        assertThat(connection.openStreamCount()).isEqualTo(0)
    ...
    fun serverWritesTrailersWithData() {
        // We buffer some outbound data and headers and confirm that the END_STREAM flag comes with
        // the
    ...
    fun clientCannotReadTrailersWithoutExhaustingStream() {
        ...
        connection.writePingAndAwaitPong()
        assertFailsWith<IllegalStateException> { stream.trailers() }
    }
    ...
    fun clientCannotReadTrailersIfTheStreamFailed() {
        ...
        val stream = connection.newStream(headerEntries("a", "artichaut"), true)
        connection.writePingAndAwaitPong()
        assertFailsWith<StreamResetException> { stream.trailers() }
    }
    ...
    fun serverClosesClientOutputStream() {
        ...
        assertFailsWith<IOException> {
            ...
            .also { expected -> assertThat(expected.message).isEqualTo("stream was reset: CANCEL") }
        // Close throws because buffered data wasn't flushed.
        assertFailsWith<IOException> { out.close() }
        assertThat(connection.openStreamCount()).isEqualTo(0)
    ...
    fun clientClosesClientInputStream() {
        ...
        val out = stream.getSink().buffer()
        source.close()
        assertFailsWith<IOException> { source.read(Buffer(), 1) }
            .also { expected -> assertThat(expected.message).isEqualTo("stream closed") }
        assertFailsWith<IOException> {
                out.writeUtf8("a")
                out.flush()
            }
            .also { expected -> assertThat(expected.message).isEqualTo("stream finished") }
        assertThat(connection.openStreamCount()).isEqualTo(0)
    ...
    fun remoteSendsRefusedStreamBeforeReplyHeaders() {
        ...
        val connection = connect(peer)
        val stream = connection.newStream(headerEntries("a", "android"), false)
        assertFailsWith<IOException> { stream.takeHeaders() }
            .also { expected ->
                assertThat(expected.message).isEqualTo("stream was reset: REFUSED_STREAM")
            }
        assertThat(connection.openStreamCount()).isEqualTo(0)
    ...
    fun close() {
        ...
        assertFailsWith<IOException> {
            ...
            .also { expected -> assertThat(expected.message).isEqualTo("stream finished") }
        assertFailsWith<IOException> { stream.getSource().read(Buffer(), 1) }
            .also { expected -> assertThat(expected.message).isEqualTo("stream was reset: CANCEL") }
    ...
    fun getResponseHeadersTimesOut() {
        ...
        assertFailsWith<InterruptedIOException> { stream.takeHeaders() }
        val elapsedNanos = System.nanoTime() - startNanos
        awaitWatchdogIdle()
        // 200ms delta
        assertThat(TimeUnit.NANOSECONDS.toMillis(elapsedNanos).toDouble()).isCloseTo(500.0, 200.0)
        assertThat(connection.openStreamCount()).isEqualTo(0)
    ...
    fun readTimesOut() {
        ...
        source.require(3)
        val startNanos = System.nanoTime()
        assertFailsWith<InterruptedIOException> { source.require(4) }
        val elapsedNanos = System.nanoTime() - startNanos
        awaitWatchdogIdle()
        // 200ms delta
        assertThat(TimeUnit.NANOSECONDS.toMillis(elapsedNanos).toDouble()).isCloseTo(500.0, 200.0)
        ...
        Thread.sleep(
            TimeUnit.NANOSECONDS.toMillis(Http2Connection.DEGRADED_PONG_TIMEOUT_NS.toLong())
        )
    ...
    fun testTruncatedDataFrame() {
        ...
        assertThat(stream.takeHeaders()).isEqualTo(headersOf("a", "android"))
        val source = stream.getSource()
        assertFailsWith<EOFException> { source.buffer().readByteString(101) }
    }
    ...
    fun blockedStreamDoesntStarveNewStream() {
        ...
        assertThat(connection.writeBytesMaximum)
            .isEqualTo(Settings.DEFAULT_INITIAL_WINDOW_SIZE.toLong())
        assertThat(stream1.writeBytesTotal).isEqualTo(Settings.DEFAULT_INITIAL_WINDOW_SIZE.toLong())
    ...
    fun remoteOmitsInitialSettings() {
        ...
        peer.play()
        val connection =
            Http2Connection.Builder(true, TaskRunner.INSTANCE).socket(peer.openSocket()).build()
        connection.start(sendConnectionPreface = false)
        val stream = connection.newStream(headerEntries("b", "banana"), false)
        assertFailsWith<IOException> { stream.takeHeaders() }
            .also { expected ->
    ...
    fun connectionUsesTaskRunner() {
        ...
        val socket = peer.openSocket()
        val connection =
            Http2Connection.Builder(true, taskRunner).socket(socket).pushObserver(IGNORE).build()
        connection.start(sendConnectionPreface = false)
     ...
     * creates more work for the watchdog and waits for that work to be executed. When it is, we
     * know work that preceded this call is complete.
     */
