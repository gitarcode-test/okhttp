...
class OkHttpClientTest {
    @RegisterExtension var platform = PlatformRule()
    ...
    fun nullInterceptorInList() {
        ...
        assertFailsWith<IllegalStateException> { builder.build() }
            .also { expected -> assertThat(expected.message).isEqualTo("Null interceptor: [null]") }
    }
    ...
    @Test
    fun testH2PriorKnowledgeOkHttpClientConstructionFallback() {
        assertFailsWith<IllegalArgumentException> {
    ...
    @Test
    fun testH2PriorKnowledgeOkHttpClientConstructionDuplicates() {
        assertFailsWith<IllegalArgumentException> {
                OkHttpClient.Builder()
    ...
    @Test
    fun testH2PriorKnowledgeOkHttpClientConstructionSuccess() {
        val okHttpClient =
    ...
    fun noSslSocketFactoryConfigured() {
        val client =
            OkHttpClient.Builder().connectionSpecs(listOf(ConnectionSpec.CLEARTEXT)).build()
        assertFailsWith<IllegalStateException> { client.sslSocketFactory }
    }
    ...
    fun nullHostileProtocolList() {
        val nullHostileProtocols =
            object : AbstractList<Protocol?>() {
                ...
                as List<Protocol>
        val client = OkHttpClient.Builder().protocols(nullHostileProtocols).build()
        assertEquals(
    ...
    fun nullProtocolInList() {
        ...
        assertFailsWith<IllegalArgumentException> {
                OkHttpClient.Builder().protocols(protocols as List<Protocol>)
            }
            .also { expected ->
    ...
    @Test
    fun spdy3IsRemovedFromProtocols() {
        val protocols =
            mutableListOf(
                Protocol.HTTP_1_1,
                Protocol.SPDY_3,
            )
        val client = OkHttpClient.Builder().protocols(protocols).build()
        assertThat(client.protocols).containsExactly(Protocol.HTTP_1_1)
    }

    @Test
    fun testProxyDefaults() {
        var client = OkHttpClient.Builder().build()
        assertThat(client.proxy).isNull()
        assertThat(client.proxySelector).isNotInstanceOf(NullProxySelector::class.java)
        client = OkHttpClient.Builder().proxy(Proxy.NO_PROXY).build()
        assertThat(client.proxy).isSameAs(Proxy.NO_PROXY)
        assertThat(client.proxySelector).isInstanceOf(NullProxySelector::class.java)
        client = OkHttpClient.Builder().proxySelector(FakeProxySelector()).build()
        assertThat(client.proxy).isNull()
        assertThat(client.proxySelector).isInstanceOf(FakeProxySelector::class.java)
    }
    ...
    fun sharesRouteDatabase() {
        ...
        assertNotSame(
            ...
            OkHttpClient.Builder().build().routeDatabase,
        )
        ...
        assertNotSame(
            client.routeDatabase,
            client.newBuilder().proxySelector(proxySelector).build().routeDatabase,
        )
        assertNotSame(
            client.routeDatabase,
    ...
    fun minWebSocketMessageToCompressNegative() {
        val builder = OkHttpClient.Builder()
        assertFailsWith<IllegalArgumentException> { builder.minWebSocketMessageToCompress(-1024) }
            .also { expected ->
                assertThat(expected.message)
