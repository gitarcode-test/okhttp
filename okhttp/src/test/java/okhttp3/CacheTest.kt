...
class CacheTest {
    ...
    private fun assertCached(
        ...
        if (responseCode == HttpURLConnection.HTTP_CLIENT_TIMEOUT) {
            ...
            // response code. In this scenario, there are 2 responses: the initial 408 and then the
            // 200
    ...
    /**
     * We've had bugs where caching and cross-protocol redirects yield class cast exceptions
     * internal to the cache because we incorrectly assumed that HttpsURLConnection was always HTTPS
     * and HttpURLConnection was always HTTP; in practice redirects mean that each can do either.
     *
    ...
    private fun requestBodyOrNull(requestMethod: String): RequestBody? {
        return if (requestMethod == "POST" || requestMethod == "PUT")
    ...
    /**
     * When the server returns a full response body we will store it and return it regardless of
     * what its Last-Modified date is. This behavior was different prior to OkHttp 3.5 when we would
     * prefer the response with the later Last-Modified date.
     *
    ...
    private fun assertNonIdentityEncodingCached(response: MockResponse) {
        ...
        // At least three request/response pairs are required because after the first request is
        // cached
    ...
    /**
     * Old implementations of OkHttp's response cache wrote header fields like ":status: 200 OK".
     * This broke our cached response parser because it split on the first colon. This regression
     * test exists to help us read these old bad cache entries.
     *
...
"""
     ...
     *   past; positive values yield dates in the future.
     */
