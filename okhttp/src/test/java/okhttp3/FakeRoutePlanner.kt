...
class FakeRoutePlanner(
    ...
    fun addPlan(): FakePlan {
        return FakePlan(nextPlanId++).also { plans += it }
    }
    ...
    inner class FakePlan(
        ...
        fun createRetry(): FakePlan {
            check(retry == null)
            return FakePlan(nextPlanId++).also { retry = it }
        }

        fun createConnectTcpNextPlan(): FakePlan {
            check(connectTcpNextPlan == null)
            return FakePlan(nextPlanId++).also { connectTcpNextPlan = it }
        }

        fun createConnectTlsNextPlan(): FakePlan {
            check(connectTlsNextPlan == null)
            return FakePlan(nextPlanId++).also { connectTlsNextPlan = it }
        }
        ...
        override fun connectTcp(): ConnectResult {
            ...
            return when {
                tcpConnectThrowable != null -> {
                    ...
                    ConnectResult(
                        this,
                        nextPlan = connectTcpNextPlan,
                        throwable = tcpConnectThrowable
                    )
                }
                canceled -> {
        ...
        override fun connectTlsEtc(): ConnectResult {
            ...
            return when {
                tlsConnectThrowable != null -> {
                    ...
                    ConnectResult(
                        this,
                        nextPlan = connectTlsNextPlan,
                        throwable = tlsConnectThrowable
                    )
                }
                canceled -> {
