...
open class CallTest {
    ...
    @RegisterExtension val testLogHandler = TestLogHandler(OkHttpClient::class.java)
    ...
    fun invalidScheme() {
        ...
        assertFailsWith<IllegalArgumentException> { requestBuilder.url("ftp://hostname/path") }
            .also { expected ->
    ...
    fun invalidPort() {
        val requestBuilder = Request.Builder()
        assertFailsWith<IllegalArgumentException> { requestBuilder.url("http://localhost:65536/") }
            .also { expected ->
    ...
    fun headResponseContentLengthIsIgnored() {
        server.enqueue(
            MockResponse.Builder().clearHeaders().addHeader("Content-Length", "100").build(),
        )
        server.enqueue(
    ...
    fun headResponseContentEncodingIsIgnored() {
        server.enqueue(
            MockResponse.Builder().clearHeaders().addHeader("Content-Encoding", "chunked").build(),
        )
    ...
    private fun postBodyRetransmittedAfterAuthorizationFail(body: String) {
        ...
        server.enqueue(MockResponse())
        val request =
            Request.Builder().url(server.url("/")).method("POST", body.toRequestBody(null)).build()
    ...
    fun delete() {
        server.enqueue(MockResponse(body = "abc"))
        val request = Request.Builder().url(server.url("/")).delete().build()
        executeSynchronously(request).assertCode(200).assertBody("abc")
        val recordedRequest = server.takeRequest()
    ...
    fun unspecifiedRequestBodyContentTypeDoesNotGetDefault() {
        server.enqueue(MockResponse())
        val request =
            Request.Builder().url(server.url("/")).method("POST", "abc".toRequestBody(null)).build()
    ...
    fun illegalToExecuteTwice() {
        ...
        val request =
            Request.Builder().url(server.url("/")).header("User-Agent", "SyncApiTest").build()
        val call = client.newCall(request)
        val response = call.execute()
        response.body.close()
        assertFailsWith<IllegalStateException> { call.execute() }
            .also { expected -> assertThat(expected.message).isEqualTo("Already Executed") }
        assertFailsWith<IllegalStateException> { call.enqueue(callback) }
            .also { expected -> assertThat(expected.message).isEqualTo("Already Executed") }
    ...
    fun get_Async() {
        ...
        val request =
            Request.Builder().url(server.url("/")).header("User-Agent", "AsyncApiTest").build()
    ...
    /**
     * Each OkHttpClient used to get its own instance of NullProxySelector, and because these
     * weren't equal their connections weren't pooled. That's a nasty performance bug!
     *
    ...
    fun connectionPoolingWithClientBuiltOffProxy() {
        client = OkHttpClient.Builder().proxy(server.toProxyAddress()).build()
        server.enqueue(MockResponse(body = "abc"))
        server.enqueue(MockResponse(body = "def"))
        server.enqueue(MockResponse(body = "ghi"))
    ...
    fun timeoutsUpdatedOnReusedConnections() {
        server.enqueue(MockResponse(body = "abc"))
        server.enqueue(
            MockResponse.Builder().body("def").throttleBody(1, 750, TimeUnit.MILLISECONDS).build(),
        )

        // First request: time out after 1s.
        client = client.newBuilder().readTimeout(Duration.ofSeconds(1)).build()
        executeSynchronously("/a").assertBody("abc")

        // Second request: time out after 250ms.
        client = client.newBuilder().readTimeout(Duration.ofMillis(250)).build()
        ...
        // The second byte of this request will be delayed by 750ms so we should time out after
        // 250ms.
        val startNanos = System.nanoTime()
        bodySource.use {
            assertFailsWith<IOException> { bodySource.readByte() }
                .also { expected ->
    ...
    fun tlsTimeoutsNotRetried() {
        ...
        client = client.newBuilder().readTimeout(Duration.ofMillis(100)).build()
            ...
            clientTestRule
                ...
                .addInterceptor(
                    Interceptor { chain: Interceptor.Chain ->
                        ...
                        assertFailsWith<IllegalStateException> { chain.proceed(chain.request()) }
                            .also { expected ->
                                assertThat(expected.message!!)
                                    .contains("please call response.close()")
                            }
                        response
                    },
                )
    ...
    /**
     * Make a request with two routes. The first route will fail because the null server connects
     * but never responds. The manual retry will succeed.
     */
    @Test
    fun readTimeoutFails() {
        ...
        client =
            client
                ...
                .build()
        val request = Request.Builder().url("http://android.com/").build()
        executeSynchronously(request).assertFailure(SocketTimeoutException::class.java)
        executeSynchronously(request).assertCode(200).assertBody("success!")
    }
    ...
    fun asyncCallEngineInitialized() {
        val c = client.newBuilder().addInterceptor(Interceptor { throw IOException() }).build()
        val request = Request.Builder().url(server.url("/")).build()
    ...
    fun reusedSinksGetIndependentTimeoutInstances() {
        ...
        val request1 = Request.Builder().url(server.url("/")).method("POST", requestBody1).build()
        ...
        val request2 = Request.Builder().url(server.url("/")).method("POST", requestBody2).build()
    ...
    fun noRecoverWhenRetryOnConnectionFailureIsFalse() {
        ...
        server.enqueue(MockResponse(body = "unreachable!"))
        client =
            client.newBuilder().dns(DoubleInetAddressDns()).retryOnConnectionFailure(false).build()
        executeSynchronously("/").assertBody("seed connection pool")
    ...
    fun recoverFromTlsHandshakeFailure_tlsFallbackScsvEnabled() {
        ...
        val supportedCiphers =
            listOf(*handshakeCertificates.sslSocketFactory().supportedCipherSuites)
        ...
        client =
            client
                ...
                .build()
        val request = Request.Builder().url(server.url("/")).build()
        assertFailsWith<SSLHandshakeException> { client.newCall(request).execute() }
        val firstSocket = clientSocketFactory.socketsCreated[0]
        assertThat(firstSocket.enabledCipherSuites).doesNotContain(tlsFallbackScsv)
        val secondSocket = clientSocketFactory.socketsCreated[1]
        assertThat(secondSocket.enabledCipherSuites).contains(tlsFallbackScsv)
    }
    ...
    fun tlsHostnameVerificationFailure() {
        ...
        val serverCertificates =
            HandshakeCertificates.Builder().heldCertificate(serverCertificate).build()
    ...
    fun anonCipherSuiteUnsupported() {
        ...
        client =
            client
                ...
                .build()
        val serverCertificates = HandshakeCertificates.Builder().build()
        server.useHttps(
            socketFactoryWithCipherSuite(serverCertificates.sslSocketFactory(), cipherSuite),
        )
        executeSynchronously("/").assertFailure(SSLHandshakeException::class.java)
    }
    ...
    fun cleartextCallsFailWhenCleartextIsDisabled() {
        ...
        server.enqueue(MockResponse())
        val request = Request.Builder().url(server.url("/")).build()
        assertFailsWith<UnknownServiceException> { client.newCall(request).execute() }
            .also { expected ->
                assertThat(expected.message)
                    .isEqualTo(
    ...
    fun httpsCallsFailWhenProtocolIsH2PriorKnowledge() {
        client = client.newBuilder().protocols(listOf(Protocol.H2_PRIOR_KNOWLEDGE)).build()
        server.useHttps(handshakeCertificates.sslSocketFactory())
        server.enqueue(MockResponse())
        val call = client.newCall(Request(server.url("/")))
        assertFailsWith<UnknownServiceException> { call.execute() }
            .also { expected ->
                assertThat(expected.message)
                    .isEqualTo("H2_PRIOR_KNOWLEDGE cannot be used with HTTPS")
            }
    }
    ...
    fun matchingPinnedCertificate() {
        ...
        val request2 = Request.Builder().url(server.url("/")).build()
        val response2 = client.newCall(request2).execute()
        assertThat(response1.handshake)
            .isNotSameAs(
                response2.handshake,
            )
    ...
    fun unmatchingPinnedCertificate() {
        ...
        val request = Request.Builder().url(server.url("/")).build()
        assertFailsWith<SSLPeerUnverifiedException> { client.newCall(request).execute() }
            .also { expected ->
    ...
    fun serverHalfClosingBeforeResponse() {
        ...
        assertFailsWith<IOException> { client.newCall(request2).execute() }
            .also { expected ->
    ...
    fun networkDropsOnConditionalGet() {
        ...
        client = client.newBuilder().dns(FakeDns()).build()
    ...
    fun redirectsDoNotIncludeTooManyCookies() {
        ...
        client = client.newBuilder().cookieJar(JavaNetCookieJar(cookieManager)).build()
    ...
    fun cancelBeforeBodyIsRead() {
        ...
        call.cancel()
        assertFailsWith<IOException> { result.get()!!.body.bytes() }
        assertThat(server.requestCount).isEqualTo(1)
    }
    ...
    fun gzip() {
        ...
        server.enqueue(
            MockResponse.Builder().body(gzippedBody).addHeader("Content-Encoding: gzip").build(),
        )
    ...
    fun gzipResponseAfterAuthenticationChallenge() {
        ...
        client =
            client.newBuilder().authenticator(RecordingOkAuthenticator("password", null)).build()
    ...
    fun rangeHeaderPreventsAutomaticGzip() {
        ...
        val request = Request.Builder().url(server.url("/")).header("Range", "bytes=0-").build()
        val call = client.newCall(request)

        // The response is not decompressed.
        val response = call.execute()
        assertThat(response.header("Content-Encoding")).isEqualTo("gzip")
        assertThat(response.body.source().readByteString())
            .isEqualTo(
    ...
    fun expect100ContinueNonEmptyRequestBody() {
        server.enqueue(
            MockResponse.Builder().add100Continue().build(),
        )
        val request =
            Request.Builder()
                .url(server.url("/"))
    ...
    fun expect100ContinueTimesOutWithoutContinue() {
        server.enqueue(MockResponse(socketPolicy = NoResponse))
        client = client.newBuilder().readTimeout(Duration.ofMillis(500)).build()
        val request =
            Request.Builder()
                .url(server.url("/"))
                .header("Expect", "100-continue")
    ...
    fun proxyUpgradeFailsWithTruncatedResponse() {
        ...
        assertFailsWith<IOException> { client.newCall(request).execute() }
            .also { expected -> assertThat(expected).hasMessage("unexpected end of stream") }
    }
    ...
    fun tooManyProxyAuthFailuresWithConnectionClose() {
        ...
        client =
            client
                ...
                .build()
        val request = Request("https://android.com/foo".toHttpUrl())
        assertFailsWith<ProtocolException> { client.newCall(request).execute() }
    }

    /**
     * Confirm that we don't send the Proxy-Authorization header from the request to the proxy
     * server. We used to have that behavior but it is problematic because unrelated requests end up
     * sharing credentials. Worse, that approach leaks proxy credentials to the origin server.
     */
    @Test
    ...
    fun serverSendsInvalidResponseHeaders() {
        server.enqueue(
            MockResponse.Builder().status("HTP/1.1 200 OK").build(),
        )
    ...
    fun serverSendsInvalidCodeTooLarge() {
        server.enqueue(
            MockResponse.Builder().status("HTTP/1.1 2147483648 OK").build(),
        )
    ...
    fun serverSendsInvalidCodeNotANumber() {
        server.enqueue(
            MockResponse.Builder().status("HTTP/1.1 00a OK").build(),
        )
    ...
    fun serverSendsUnnecessaryWhitespace() {
        server.enqueue(
            MockResponse.Builder().status(" HTTP/1.1 200 OK").build(),
        )
    ...
    fun requestHeaderNameWithSpaceForbidden() {
        assertFailsWith<IllegalArgumentException> { Request.Builder().addHeader("a b", "c") }
            .also { expected ->
                assertThat(expected.message)
    ...
    fun connectFails() {
        server.shutdown()
        executeSynchronously("/").assertFailure(IOException::class.java)
    }
    ...
    fun requestBodySurvivesRetries() {
        ...
        client =
            client
                .newBuilder()
                .proxySelector(
                    FakeProxySelector().addProxy(server2.toProxyAddress()).addProxy(Proxy.NO_PROXY),
                )
                .build()
        server2.shutdown()
        val request =
    ...
    fun invalidHost() {
        ...
        executeSynchronously(request).assertFailure(UnknownHostException::class.java)
    }
    ...
    fun ipv6HostHasSquareBracesHttp1() {
        ...
        val url = server.url("/").newBuilder().host("::1").port(port).build()
    ...
    fun ipv6HostHasSquareBracesHttp2() {
        ...
        assertThat(response.body.string()).isEqualTo("response body")
        val connect = server.takeRequest()
        assertThat(connect.requestLine)
            .isEqualTo(
                "CONNECT [::1]:$port HTTP/1.1",
            )
        assertThat(connect.headers["Host"])
            .isEqualTo(
                "[::1]:$port",
            )
        assertThat(connect.headers[":authority"]).isNull()
        ...
        assertThat(get.headers[":authority"])
            .isEqualTo(
                "[::1]:$port",
            )
        assertThat(get.requestUrl).isEqualTo(url)
    ...
    fun ipv4IpHostHasNoSquareBracesHttp1() {
        ...
        val port = server.port
        val url = server.url("/").newBuilder().host("127.0.0.1").port(port).build()
        val request = Request(url)
        val response = client.newCall(request).execute()
        assertThat(response.body.string()).isEqualTo("response body")
        val connect = server.takeRequest()
    ...
    fun hostnameRequestHostHasNoSquareBracesHttp1() {
        ...
        val port = server.port
        val url = server.url("/").newBuilder().host("any-host-name").port(port).build()
        val request = Request(url)
        val response = client.newCall(request).execute()
        assertThat(response.body.string()).isEqualTo("response body")
        val connect = server.takeRequest()
    ...
    fun failedAuthenticatorReleasesConnection() {
        ...
        client =
            client
                ...
                .build()
        val request = Request(server.url("/"))
        executeSynchronously(request).assertFailure(IOException::class.java)
    ...
    fun failedProxyAuthenticatorReleasesConnection() {
        ...
        client =
            client
                .newBuilder()
                .proxyAuthenticator { _: Route?, _: Response -> throw IOException("IOException!") }
                .build()
        val request = Request(server.url("/"))
        executeSynchronously(request).assertFailure(IOException::class.java)
        assertThat(client.connectionPool.idleConnectionCount()).isEqualTo(1)
    ...
    fun httpsWithIpAddress() {
        ...
        // Make a request.
        server.enqueue(MockResponse())
        val url = server.url("/").newBuilder().host(localIpAddress).build()
        val request = Request(url)
        executeSynchronously(request).assertCode(200)
    ...
    fun postWithFileNotFound() {
        ...
        client = client.newBuilder().dns(DoubleInetAddressDns()).build()
        executeSynchronously(request).assertFailure(FileNotFoundException::class.java)
            ...
            Request(
                ...
                body =
                    object : RequestBody() {
                        ...
                        override fun writeTo(sink: BufferedSink) {
                            sink
                                .flush() // For determinism, always send a partial request to the
                                         // server.
    ...
    fun cancelAfterResponseBodyEnd() {
        ...
        server.enqueue(MockResponse(body = "abc"))
        server.enqueue(MockResponse(body = "def"))
        client = client.newBuilder().protocols(listOf(Protocol.HTTP_1_1)).build()
        val cancelClient =
            client
                .newBuilder()
                .eventListener(
    ...
    fun priorResponseBodyNotReadable() {
        ...
        assertFailsWith<IllegalStateException> { response.priorResponse?.body?.string() }
            .also { expected ->
    ...
    private fun makeFailingCall() {
        ...
        val nonRetryingClient = client.newBuilder().retryOnConnectionFailure(false).build()
        val call =
        ...
        assertFailsWith<IOException> { call.execute() }
            .also { expected -> assertThat(expected.message).isEqualTo("write body fail!") }
    }
