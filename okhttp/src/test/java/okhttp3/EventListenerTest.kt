...
class EventListenerTest {
    ...
    fun failedCallEventSequence() {
        server.enqueue(
            MockResponse.Builder().headersDelay(2, TimeUnit.SECONDS).build(),
        )
        ...
        assertFailsWith<IOException> { call.execute() }
            .also { expected -> assertThat(expected.message).isIn("timeout", "Read timed out") }
        assertThat(listener.recordedEventTypes())
            .containsExactly(
                "CallStart",
                "ProxySelectStart",
    ...
    fun failedDribbledCallEventSequence() {
        ...
        assertFailsWith<IOException> { response.body.string() }
            .also { expected -> assertThat(expected.message).isEqualTo("unexpected end of stream") }
        assertThat(listener.recordedEventTypes())
            .containsExactly(
                "CallStart",
                "ProxySelectStart",
    ...
    fun canceledCallEventSequence() {
        ...
        call.cancel()
        assertFailsWith<IOException> { call.execute() }
            .also { expected -> assertThat(expected.message).isEqualTo("Canceled") }
        assertThat(listener.recordedEventTypes())
            .containsExactly(
                "Canceled",
                "CallStart",
    ...
    fun multipleDnsLookupsForSingleCall() {
        ...
        val call =
            client.newCall(
                Request.Builder().url("http://fakeurl:" + server.port).build(),
            )
        val response = call.execute()
        assertThat(response.code).isEqualTo(200)
        response.body.close()
    ...
    fun failedDnsLookup() {
        ...
        val call =
            client.newCall(
                Request.Builder().url("http://fakeurl/").build(),
            )
        assertFailsWith<IOException> { call.execute() }
        listener.removeUpToEvent<DnsStart>()
    ...
    fun failedConnect() {
        enableTlsWithTunnel()
        server.enqueue(
            MockResponse.Builder().socketPolicy(FailHandshake).build(),
        )
        val call =
            client.newCall(
                Request.Builder().url(server.url("/")).build(),
    ...
    fun successfulHttpProxyConnect() {
        ...
        val call =
            client.newCall(
                Request.Builder().url("http://www.fakeurl").build(),
            )
        val response = call.execute()
        assertThat(response.code).isEqualTo(200)
        response.body.close()
    ...
    fun authenticatingTunnelProxyConnect() {
        ...
        server.enqueue(
            MockResponse.Builder().inTunnel().build(),
        )
        server.enqueue(MockResponse())
        client =
            client
    ...
    fun noConnectionFoundOnFollowUp() {
        server.enqueue(
            MockResponse.Builder().code(301).addHeader("Location", "/foo").build(),
        )
        server.enqueue(
            MockResponse.Builder().body("ABC").build(),
        )
    ...
    fun pooledConnectionFound() {
        ...
        assertThat(connectionAcquired2.connection)
            .isSameAs(
    ...
    fun emptyResponseBodyConnectionClose() {
        server.enqueue(
            MockResponse.Builder().addHeader("Connection", "close").body("").build(),
        )
        val call =
            client.newCall(
                Request.Builder().url(server.url("/")).build(),
    ...
    private fun requestBodyFail(expectedProtocol: Protocol?) {
        ...
        val call =
            client.newCall(
                Request.Builder().url(server.url("/")).post(request).build(),
            )
        assertFailsWith<IOException> { call.execute() }
    ...
    fun requestBodyMultipleFailuresReportedOnlyOnce() {
        ...
        val call =
            client.newCall(
                Request.Builder().url(server.url("/")).post(requestBody).build(),
            )
        assertFailsWith<IOException> { call.execute() }
        assertThat(listener.recordedEventTypes())
            .containsExactly(
    ...
    private fun requestBodySuccess(
        ...
        server.enqueue(
            MockResponse.Builder().code(200).body("World!").build(),
        )
        val call =
            client.newCall(
                Request.Builder().url(server.url("/")).post(body!!).build(),
            )
        val response = call.execute()
     ...
     *
     * We've had bugs where we report an event when we request data rather than when the data
    ...
    private fun timeToFirstByte() {
        ...
        call.execute().use { response -> assertThat(response.body.string()).isEqualTo("fghijk") }
    ...
    fun applicationInterceptorProceedsMultipleTimes() {
        ...
        client =
            client
                ...
                .addInterceptor(
                    Interceptor { chain: Interceptor.Chain? ->
                        chain!!.proceed(chain.request()).use { a ->
    ...
    fun timeToFirstByteGapBetweenResponseHeaderStartAndEnd() {
        ...
        val request =
            Request.Builder()
                ...
                .build()
        val call = client.newCall(request)
        call.execute().use { response -> assertThat(response.body.string()).isEqualTo("") }
    ...
    fun conditionalCache() {
        enableCache()
        server.enqueue(
            MockResponse.Builder().addHeader("ETag", "v1").body("abc").build(),
        )
        server.enqueue(
