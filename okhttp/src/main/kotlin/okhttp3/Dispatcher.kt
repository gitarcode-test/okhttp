...
class Dispatcher() {
    ...
    var maxRequests = 64
        ...
        set(maxRequests) {
            ...
            this.withLock { field = maxRequests }
            promoteAndExecute()
        }
    ...
    var maxRequestsPerHost = 5
        ...
        set(maxRequestsPerHost) {
            ...
            this.withLock { field = maxRequestsPerHost }
            promoteAndExecute()
        }
     ...
     *
     * Note: The time at which a [call][Call] is considered idle is different depending on whether
     * it was run [asynchronously][Call.enqueue] or [synchronously][Call.execute]. Asynchronous
     * calls become idle after the [onResponse][Callback.onResponse] or
    ...
    internal fun enqueue(call: AsyncCall) {
        this.withLock {
            ...
            // Mutate the AsyncCall so that it shares the AtomicInteger of an existing running call
            // to
            // the same host.
    ...
    internal fun executed(call: RealCall) = this.withLock { runningSyncCalls.add(call) }
    ...
    fun runningCalls(): List<Call> =
        this.withLock {
            return Collections.unmodifiableList(
                runningSyncCalls + runningAsyncCalls.map { it.call }
            )
        }
