...
abstract class RequestBody {
    ...
    /**
     * Returns the number of bytes that will be written to sink in a call to [writeTo], or -1 if
     * that count is unknown.
     */
    @Throws(IOException::class) open fun contentLength(): Long = commonContentLength()
     ...
     * receiving. With duplex the request and response may be interleaved! That is, request body
     * bytes may be sent after response headers or body bytes have been received.
     *
     ...
     * bodies may only be used with HTTP/2. Calls to HTTP/1 servers will fail before the HTTP
     * request is transmitted. If you cannot ensure that your client and server both support HTTP/2,
    ...
    /**
     * Returns true if this body expects at most one call to [writeTo] and can be transmitted at
     * most once. This is typically used when writing the request body is destructive and it is not
    ...
    companion object {
        /**
         * Returns a new request body that transmits this string. If [contentType] is non-null and
        ...
        fun FileDescriptor.toRequestBody(contentType: MediaType? = null): RequestBody {
            return object : RequestBody() {
                ...
                override fun writeTo(sink: BufferedSink) {
                    FileInputStream(this@toRequestBody).use { sink.buffer.writeAll(it.source()) }
                }
            }
        }
        ...
        /**
         * Returns a gzip version of the RequestBody, with compressed payload. This is not automatic
         * as not all servers support gzip compressed requests.
         *
         * ```
