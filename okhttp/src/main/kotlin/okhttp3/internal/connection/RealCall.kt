 ...
 *
 * This class supports [asynchronous canceling][cancel]. This is intended to have the smallest blast
 * radius possible. If an HTTP/2 stream is active, canceling will cancel that stream but not the
 * other streams sharing its connection. But if the TLS handshake is still in progress then
...
class RealCall(
    ...
    /**
     * This is the same value as [exchange], but scoped to the execution of the network
     * interceptors. The [exchange] field is assigned to null when its streams end, which may be
     * before or after the network interceptors return.
     */
    internal var interceptorScopedExchange: Exchange? = null
        private set

    // These properties are guarded by [lock]. They are typically only accessed by the thread
    // executing
    ...
    /**
     * Prepare for a potential trip through all of this call's network interceptors. This prepares
     * to find an exchange to carry the request.
     *
        ...
        if (newRoutePlanner) {
            val routePlanner =
                RealRoutePlanner(
                    ...
                    connectionUser =
                        CallConnectionUser(this, connectionPool.connectionListener, chain),
                    routeDatabase = client.routeDatabase,
                )
            this.exchangeFinder =
                when {
                    client.fastFallback ->
                        FastFallbackExchangeFinder(routePlanner, client.taskRunner)
    ...
    /**
     * Releases resources held with the request or response of [exchange]. This should be called
    ...
    /**
     * @param closeExchange true if the current exchange should be closed because it will not be
     *   used. This is usually due to either an exception or a retry.
     */
    internal fun exitNetworkInterceptorExchange(closeExchange: Boolean) {
        this.withLock { check(expectMoreExchanges) { "released" } }
    ...
    inner class AsyncCall(
        ...
        /**
         * Attempt to enqueue this async call on [executorService]. This will attempt to clean up if
         * the executor has been shut down by reporting the call as failed.
         */
