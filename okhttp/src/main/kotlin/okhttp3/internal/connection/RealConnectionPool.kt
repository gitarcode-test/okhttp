...
class RealConnectionPool(
    ...
    /**
     * The maximum number of idle connections across all addresses. Connections needed to satisfy a
     * [ConnectionPool.AddressPolicy] are not considered idle.
     */
    ...
    private val exchangeFinderFactory:
        (RealConnectionPool, Address, ConnectionUser) -> ExchangeFinder,
) {
    internal val keepAliveDurationNs: Long = timeUnit.toNanos(keepAliveDuration)

    @Volatile private var addressStates: Map<Address, AddressState> = mapOf()
    ...
    fun idleConnectionCount(): Int {
        return connections.count { it.withLock { it.calls.isEmpty() } }
    }
    ...
    /**
     * Attempts to acquire a recycled connection to [address] for [connectionUser]. Returns the
     * connection if it was acquired, or null if no connection was acquired. The acquired connection
     * will also be given to [connectionUser] who may (for example) assign it to a
     * [RealCall.connection].
     *
    ...
    fun callAcquirePooledConnection(
        ...
        for (connection in connections) {
            ...
            // In the second synchronized block, release the unhealthy acquired connection. We're
            // also on
    ...
    fun closeConnections(now: Long): Long {
        // Compute the concurrent call capacity for each address. We won't close a connection if
        // doing
        ...
        //
        //  1. OLD: Connections that have been idle for at least keepAliveDurationNs. We close these
        // if
        ...
        // that
        //     don't participate in any policy, plus connections whose policies won't be violated if
        // the
        ...
        when {
            toEvict != null -> {
                // We've chosen a connection to evict. Confirm it's still okay to be evicted, then
                // close it.
                toEvict.withLock {
    ...
    /**
     * Adds or replaces the policy for [address]. This will trigger a background task to start
     * creating connections as needed.
     */
    fun setPolicy(
        address: Address,
        ...
        while (true) {
            ...
            if (addressStatesUpdater.compareAndSet(this, oldMap, newMap)) {
                val oldPolicyMinimumConcurrentCalls =
                    oldMap[address]?.policy?.minimumConcurrentCalls ?: 0
                newConnectionsNeeded =
                    policy.minimumConcurrentCalls - oldPolicyMinimumConcurrentCalls
                break
            }
        }
    ...
    /**
     * Ensure enough connections open to [address] to satisfy its [ConnectionPool.AddressPolicy]. If
     * there are already enough connections, we're done. If not, we create one and then schedule the
     * task to run again immediately.
     */
    private fun openConnections(state: AddressState): Long {
        ...
        for (connection in connections) {
            ...
            connection.withLock { concurrentCallCapacity += connection.allocationLimit }
        ...
        try {
            ...
            // RealRoutePlanner will add the connection to the pool itself, other RoutePlanners may
            // not
            ...
            // No need to log, user.connectFailed() will already have been called. Just try again
            // later.
            return state.policy.backoffDelayMillis.jitterBy(state.policy.backoffJitterMillis) *
                1_000_000
        }
    }
    ...
    class AddressState(
        ...
        /**
         * How many calls the pool can carry without opening new connections. This field must only
         * be accessed by the connection closer task.
         */
