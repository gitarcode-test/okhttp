...
class RealConnection(
     ...
     * true when removing a connection from the pool; otherwise a racing caller might get it from
     * the pool when it shouldn't. Symmetrically, this must always be checked before returning a
    ...
    /**
     * If true, this connection may not be used for coalesced requests. These are requests that
     * could share the same connection without sharing the same hostname.
     */
    ...
    override fun noNewExchanges() {
        this.withLock { noNewExchanges = true }
    ...
    internal fun noCoalescedConnections() {
        this.withLock { noCoalescedConnections = true }
    }

    internal fun incrementSuccessCount() {
        this.withLock { successCount++ }
    }
    ...
    private fun startHttp2() {
        ...
        val flowControlListener =
            connectionListener as? FlowControlListener ?: FlowControlListener.None
        val http2Connection =
     ...
     * can't coalesce connections that use a proxy, since proxies don't tell us the origin server's
     * IP address.
     */
    ...
    /**
     * Track a failure using this connection. This may prevent both the connection and its route
     * from being used for future exchanges.
     */
