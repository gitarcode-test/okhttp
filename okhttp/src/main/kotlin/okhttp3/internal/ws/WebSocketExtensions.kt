...
data class WebSocketExtensions(
    ...
    companion object {
        ...
        fun parse(responseHeaders: Headers): WebSocketExtensions {
            // Note that this code does case-insensitive comparisons, even though the spec doesn't
            // specify
            ...
            for (i in 0 until responseHeaders.size) {
                if (
                    !responseHeaders.name(i).equals(HEADER_WEB_SOCKET_EXTENSION, ignoreCase = true)
                ) {
                            ...
                            while (pos < extensionEnd) {
                                ...
                                val value =
                                    if (equals < parameterEnd) {
                                        header
                                            .trimSubstring(equals + 1, parameterEnd)
                                            .removeSurrounding("\"")
                                    } else {
                                        null
                                    }
