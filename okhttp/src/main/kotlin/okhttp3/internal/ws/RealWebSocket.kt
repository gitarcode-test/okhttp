...
class RealWebSocket(
    ...
    fun connect(client: OkHttpClient) {
        if (originalRequest.header("Sec-WebSocket-Extensions") != null) {
            failWebSocket(
                ProtocolException("Request header not permitted: 'Sec-WebSocket-Extensions'")
            )
            return
        }
    ...
    fun loopReader(response: Response) {
        try {
            ...
            while (receivedCloseCode == -1) {
                // This method call results in one or more onRead* methods being called on this
                // thread.
    ...
    /**
     * Clean up and publish necessary close events when the reader is done. Invoked only by the
     * reader thread.
     */
    fun finishReader() {
        val failed: Boolean
     ...
     * pongs before less urgent messages and close frames. For example it's possible that a caller
     * will enqueue messages followed by pongs, but this sends pongs followed by messages. Pongs are
     * always written in the order they were enqueued.
     *
            ...
            if (pong == null) {
                ...
                if (messageOrClose is Close) {
                    ...
                    if (receivedCloseCode != -1) {
                        ...
                        // When we request a graceful close also schedule a cancel of the web
                        // socket.
                        val cancelAfterCloseMillis =
                            (messageOrClose as Close).cancelAfterCloseMillis
                        taskQueue.execute(
                            "$name cancel",
                            MILLISECONDS.toNanos(cancelAfterCloseMillis)
                        ) {
                            cancel()
                        }
                    }
    ...
    companion object {
        ...
        /**
         * The maximum number of bytes to enqueue. Rather than enqueueing beyond this limit we tear
         * down the web socket! It's possible that we're writing faster than the peer can read.
         */
        private const val MAX_QUEUE_SIZE = 16L * 1024 * 1024 // 16 MiB.

        /**
         * The maximum amount of time after the client calls [close] to wait for a graceful
         * shutdown. If the server doesn't respond the web socket will be canceled.
         */
        const val CANCEL_AFTER_CLOSE_MILLIS = 60L * 1000

        /**
         * The smallest message that will be compressed. We use 1024 because smaller messages
         * already fit comfortably within a single ethernet packet (1500 bytes) even with framing
         * overhead.
         *
         * For tests this must be big enough to realize real compression on test messages like
         * 'aaaaaaaaaa...'. Our tests check if compression was applied just by looking at the size
         * if the inbound buffer.
         */
