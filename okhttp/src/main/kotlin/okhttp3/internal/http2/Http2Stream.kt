...
internal constructor(
     ...
     * headers have been received. If the returned list contains multiple blocks of headers the
     * blocks will be delimited by 'null'.
     *
    ...
    fun trailers(): Headers {
        this.withLock {
            if (
                source.finished && source.receiveBuffer.exhausted() && source.readBuffer.exhausted()
            ) {
     ...
     *
     * @throws IllegalStateException if this stream was initiated by the peer and a [writeHeaders]
    ...
    /**
     * Returns true if read timeouts should be enforced while reading response headers or body
     * bytes. We always do timeouts in the HTTP server role. For clients, we only do timeouts after
     * the request is transmitted. This is only interesting for duplex calls where the request and
    ...
    internal constructor(
         ...
         * beyond those already in the buffer.
         */
        ...
        /**
         * Received trailers. Null unless the server has provided trailers. Undefined until the
         * stream is exhausted. Guarded by Http2Stream.this.
         */
        ...
        override fun read(
            ...
            while (true) {
                            ...
                            if (
                                errorExceptionToDeliver == null &&
                                    unacknowledgedBytesRead >=
                                        connection.okHttpSettings.initialWindowSize / 2
                            ) {
                ...
                connection.flowControlListener.receivingStreamWindowChanged(
                    id,
                    readBytes,
                    readBuffer.size
                )
                ...
                if (errorExceptionToDeliver != null) {
                    // We defer throwing the exception until now so that we can refill the
                    // connection
                    // flow-control window. This is necessary because we don't transmit window
                    // updates until
                    // the application reads the data. If we throw this prior to updating the
                    // connection
                    // flow-control window, we risk having it go to 0 preventing the server from
                    // sending data.
        ...
        /**
         * Accept bytes on the connection's reader thread. This function avoids holding locks while
         * it performs blocking reads for the incoming bytes.
         */
        @Throws(IOException::class)
        internal fun receive(
            ...
            while (remainingByteCount > 0L) {
                ...
                // Move the received data to the read buffer to the reader can read it. If this
                // source has
    ...
    internal fun cancelStreamIfNecessary() {
        ...
        if (cancel) {
            // RST this stream to prevent additional data from being sent. This is safe because the
            // input
            // stream is closed (we won't use any further bytes) and the output stream is either
            // finished
    ...
    internal inner class FramingSink(
        ...
        override fun flush() {
            ...
            this@Http2Stream.withLock { checkOutNotClosed() }
        ...
        override fun close() {
            ...
            if (!sink.finished) {
                // We have 0 or more frames of data, and 0 or more frames of trailers. We need to
                // send at
                // least one frame with the END_STREAM flag set. That must be the last frame, and
                // the
    ...
    /**
     * Like [Object.wait], but throws an [InterruptedIOException] when interrupted instead of the
     * more awkward [InterruptedException].
     */
    ...
    /**
     * The Okio timeout watchdog will call [timedOut] if the timeout is reached. In that case we
     * close the stream (asynchronously) which will notify the waiting thread.
     */
