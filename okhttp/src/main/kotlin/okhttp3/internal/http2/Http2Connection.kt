...
class Http2Connection internal constructor(builder: Builder) : Closeable {
    ...
    val okHttpSettings =
        Settings().apply {
            ...
            // client,
            // set the flow control window to 16MiB.  This avoids thrashing window updates every
            // 64KiB, yet
    ...
    init {
        if (builder.pingIntervalMillis != 0) {
            val pingIntervalNanos =
                TimeUnit.MILLISECONDS.toNanos(builder.pingIntervalMillis.toLong())
     ...
     * endpoint to gracefully stop accepting new requests without harming previously established
     * streams.
     */
    @Throws(IOException::class)
     ...
     * called after [Builder.build] for all new connections.
     *
     * @param sendConnectionPreface true to send connection preface frames. This should always be
     *   true except for in tests that don't check for a connection preface.
     ...
     * an async task to do so.
     */
    inner class ReaderRunnable
    internal constructor(
        ...
        override fun headers(
            ...
            this@Http2Connection.withLock {
                ...
                if (stream == null) {
                    ...
                    // Use a different task queue for each stream because they should be handled in
                    // parallel.
         ...
         * acknowledged! In particular, we shouldn't send frames that assume a new
         * `initialWindowSize` until we send the frame that acknowledges this new size.
         *
         * Since we can't ACK settings on the current reader thread (the reader thread can't write)
         * we execute all peer settings logic on the writer thread. This relies on the fact that the
         * writer task queue won't reorder tasks; otherwise settings could be applied in the
         * opposite order than received.
         */
        fun applyAndAckSettings(
            ...
            if (streamsToNotify != null) {
                for (stream in streamsToNotify!!) {
                    stream.withLock { stream.addBytesToWriteWindow(delta) }
                }
            }
        }
        ...
        override fun windowUpdate(
            ...
            if (streamId == 0) {
                ...
                if (stream != null) {
                    stream.withLock { stream.addBytesToWriteWindow(windowSizeIncrement) }
                }
            }
        }
    ...
    internal fun pushedStream(streamId: Int): Boolean {
        ...
        pushQueue.execute("$connectionName[$streamId] onRequest") {
            ...
            ignoreIoExceptions {
                if (cancel) {
                    ...
                    this.withLock { currentPushRequests.remove(streamId) }
                }
            }
        }
    }
    ...
    abstract class Listener {
         ...
         * [replying to the stream][Http2Stream.writeHeaders] or [closing it][Http2Stream.close].
         ...
         *
         * Methods to this method may be made concurrently with [onStream]. But a calls to this
         * method are serialized.
         */
        open fun onSettings(
