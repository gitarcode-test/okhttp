...
class Http2Reader(
    ...
    fun readConnectionPreface(handler: Handler) {
        if (client) {
            ...
            if (logger.isLoggable(FINE))
                logger.fine(format("<< CONNECTION ${connectionPreface.hex()}"))
            if (CONNECTION_PREFACE != connectionPreface) {
                throw IOException(
                    "Expected a connection header but was ${connectionPreface.utf8()}"
                )
            }
        }
    }
    ...
    interface Handler {
         ...
         * Frames that trigger this are HEADERS and PUSH_PROMISE.
         *
         ...
         * lastGoodStreamId` on a new connection. In- flight streams with `ID <= lastGoodStreamId`
         * can only be replayed on a new connection if they are idempotent.
         *
         * @param lastGoodStreamId the last stream ID the peer processed before sending this
         *   message. If [lastGoodStreamId] is zero, the peer processed no frames.
         ...
         * `promisedStreamId` to which response frames will be delivered. Push promise frames are
         ...
         * are available from a different network location or protocol configuration.
         *
         * See [alt-svc][alt_svc].
         *
         ...
         *   alternate service is the origin of the stream. When zero, the origin is specified in
         *   the `origin` parameter.
         * @param origin when present, the [origin](http://tools.ietf.org/html/rfc6454) is typically
         *   represented as a combination of scheme, host and port. When empty, the origin is that
         *   of the `streamId`.
