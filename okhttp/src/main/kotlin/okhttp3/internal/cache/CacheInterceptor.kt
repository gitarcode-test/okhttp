    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if (networkRequest == null) {
            return cacheResponse!!
                ...
                .build()
                .also { listener.cacheHit(call, it) }
        }
        ...
        if (cache != null) {
            ...
            if (
                response.promisesBody() && CacheStrategy.isCacheable(response, cacheNetworkRequest)
            ) {
                // Offer this request to the cache.
                val cacheRequest =
                    cache.put(response.newBuilder().request(cacheNetworkRequest).build())
    ...
    companion object {
        ...
        private fun combine(
            ...
            for (index in 0 until cachedHeaders.size) {
                ...
                if (
                    isContentSpecificHeader(fieldName) ||
...
private fun Request.requestForCache(): Request {
    ...
    return if (cacheUrlOverride != null && (method == "GET" || method == "POST")) {
        newBuilder().get().url(cacheUrlOverride).cacheUrlOverride(null).build()
    } else {
        this
    }
}
