...
class DiskLruCache(
    ...
    internal fun removeEntry(entry: Entry): Boolean {
        ...
        if (!civilizedFileSystem) {
            if (entry.lockingSourceCount > 0) {
                // Mark this entry as 'DIRTY' so that if the process crashes this entry won't be
                // used.
                journalWriter?.let {
                    it.writeUtf8(DIRTY)
                    it.writeByte(' '.code)
     ...
     * exceptions.
     *
     * **The caller must [close][Snapshot.close]** each snapshot returned by [Iterator.next].
    ...
    internal constructor(
         ...
         * a single published snapshot. If we opened streams lazily then the streams could come from
         * different edits.
         */
