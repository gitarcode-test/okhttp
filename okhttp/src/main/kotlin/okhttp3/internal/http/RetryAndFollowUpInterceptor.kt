...
class RetryAndFollowUpInterceptor(private val client: OkHttpClient) : Interceptor {
        ...
        while (true) {
            ...
            try {
                ...
                try {
                    ...
                    // An attempt to communicate with a server failed. The request may have been
                    // sent.
                    if (
                        !recover(
                            e,
     ...
     * `e` is recoverable, or false if the failure is permanent. Requests with a body can only be
     * recovered if the body is buffered or if the failure occurred before the request has been
     * sent.
     */
    private fun recover(
        e: IOException,
    ...
    private fun followUpRequest(
        ...
        when (responseCode) {
            ...
            HTTP_CLIENT_TIMEOUT -> {
                // 408's are rare in practice, but some servers like HAProxy use this response code.
                // The
                // spec says that we may repeat the request without modifications. Modern browsers
                // also
            ...
            HTTP_MISDIRECTED_REQUEST -> {
                // OkHttp can coalesce HTTP/2 connections even if the domain names are different.
                // See
                // RealConnection.isEligible(). If we attempted this and the server returned HTTP
                // 421, then
