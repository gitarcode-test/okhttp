...
private constructor(
     ...
     * This is December 31, 9999 if the cookie is not [persistent], in which case it will expire at
     ...
     * - "None": the cookie is always sent. The "Secure" attribute must also be set when setting
     *   this value.
     */
    ...
    companion object {
        ...
        /**
         * Attempt to parse a `Set-Cookie` HTTP header value [setCookie] as a cookie. Returns null
         * if [setCookie] is not a well-formed cookie.
         */
        @JvmStatic
        fun parse(
            url: HttpUrl,
        ...
        internal fun parse(
            ...
            if (
                urlHost.length != domain.length &&
                    PublicSuffixDatabase.get().getEffectiveTldPlusOne(domain) == null
            ) {
                return null
            }

            // If the path is absent or didn't start with '/', use the default path. It's a string
            // like
            ...
            while (pos < limit) {
                ...
                when {
                    ...
                    month == -1 && matcher.usePattern(MONTH_PATTERN).matches() -> {
                        ...
                        month =
                            MONTH_PATTERN.pattern().indexOf(monthString) /
                                4 // Sneaky! jan=1, dec=12.
                    }
        ...
        /**
         * Returns the index of the next date character in `input`, or if `invert` the index of the
         * next non-date character in `input`.
         */
        ...
        private fun parseMaxAge(s: String): Long {
            try {
                ...
                // Check if the value is an integer (positive or negative) that's too big for a
                // long.
