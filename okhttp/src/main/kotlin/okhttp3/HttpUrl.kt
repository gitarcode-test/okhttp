 ...
 * `http://ho™ail.com`. To mitigate this, the single character (™) maps to the string (tm). There is
 * similar policy for all of the 1.1 million Unicode code points. Note that some code points such as
 * "\ud83c\udf69" are not mapped and cannot be used in a hostname.
 *
...
private constructor(
    ...
    /**
     * The entire path of this URL encoded for use in HTTP resource resolution. The returned path
     * will start with `"/"`.
     *
     * | URL                     | `encodedPath()` |
     * |-------------------------|-----------------|
    ...
    /**
     * The number of query parameters in this URL, like 2 for `http://host/?a=apple&b=banana`. If
     * this URL has no query this is 0. Otherwise it is one more than the number of `"&"` separators
     * in the query.
     *
     * | URL                               | `querySize()` |
     * |-----------------------------------|---------------|
     ...
     * For example this returns `["banana"]` for `queryParameterValue("b")` on
     ...
     * `queryParameterName(0)` on `http://host/?a=apple&b=banana`. This throws if `index` is not
     * less than the [query size][querySize].
     *
    ...
    class Builder {
        ...
        fun build(): HttpUrl {
            @Suppress(
                "UNCHECKED_CAST"
            ) // percentDecode returns either List<String?> or List<String>.
            return HttpUrl(
                ...
                queryNamesAndValues =
                    encodedQueryNamesAndValues?.map { it?.percentDecode(plusIsSpace = true) },
        ...
        internal fun parse(
            ...
            if (slashCount >= 2 || base == null || base.scheme != this.scheme) {
                    ...
                    when (c) {
                        ...
                        '#'.code -> {
                            ...
                            if (portColonOffset + 1 < componentDelimiterOffset) {
                                host =
                                    input
                                        .percentDecode(pos = pos, limit = portColonOffset)
                                        .toCanonicalHost()
                                port =
                ...
                // This is a relative link. Copy over all authority components. Also maybe the path
                // & query.
         ...
         * means the encoded path will have a trailing '/'.
         *
        ...
        /**
         * Cuts this string up into alternating parameter names and values. This divides a query
    ...
    companion object {
        ...
        /**
         * Returns a new `HttpUrl` representing `url` if it is a well-formed HTTP or HTTPS URL, or
         * null if it isn't.
         */
        @JvmStatic
        @JvmName("parse")
