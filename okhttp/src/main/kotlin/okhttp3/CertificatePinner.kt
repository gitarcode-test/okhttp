 ...
 *   number of prefixes (`us-west.www.publicobject.com`, `www.publicobject.com`) including no prefix
 *   at all (`publicobject.com`). For most applications this is the best way to configure
 *   certificate pinning.
 * * **Exactly one subdomain**: Use a single asterisk like `*.publicobject.com` to match exactly one
 *   prefix (`www.publicobject.com`, `api.publicobject.com`). Be careful with this approach as no
 *   pinning will be enforced if additional prefixes are present, or if no prefixes are present.
 *
...
internal constructor(
     ...
     * `peerCertificates`. Does nothing if there are no certificates pinned for `hostname`. OkHttp
     * calls this after a successful TLS handshake, but before the connection is used.
     *
    ...
    fun findMatchingPins(hostname: String): List<Pin> =
        pins.filterList { matchesHostname(hostname) }
    ...
    class Builder {
         ...
         *   Key Info, base64-encoded and prefixed with either `sha256/` or `sha1/`.
         */
        fun add(
            pattern: String,
            vararg pins: String,
