...
internal object CertificateAdapters {
     ...
     * > CAs conforming to this profile MUST always encode certificate validity dates through the
     * > year 2049 as UTCTime; certificate validity dates in 2050 or later MUST be encoded as
     * > GeneralizedTime.
     */
    ...
    private val algorithmParameters: DerAdapter<Any?> =
        Adapters.usingTypeHint { typeHint ->
            when (typeHint) {
                // This type is pretty strange. The spec says that for certain algorithms we must
                // encode null
     ...
     * ```
     *
     * The first property of the pair is the adapter that was used, the second property is the
     * value.
     */
    ...
    internal val tbsCertificate: BasicDerAdapter<TbsCertificate> =
        Adapters.sequence(
            "TBSCertificate",
            Adapters.INTEGER_AS_LONG.withExplicitBox(tag = 0L)
                // v1 == 0.
