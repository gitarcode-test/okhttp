...
internal object Adapters {
        ...
        BasicDerAdapter(
            ...
            codec =
                object : BasicDerAdapter.Codec<Boolean> {
                    override fun decode(reader: DerReader): Boolean {
    ...
    /**
     * Returns a composite adapter for a struct or data class. This may be used for both SEQUENCE
     * and SET types.
     *
     ...
     *
     * If the hint is unknown [chooser] should return null which will cause the value to be decoded
    ...
    fun usingTypeHint(chooser: (Any?) -> DerAdapter<*>?): DerAdapter<Any?> {
        return object : DerAdapter<Any?> {
            ...
            override fun toDer(
                ...
                // If we don't understand this hint, encode the body as a byte string. The byte
                // string
     ...
     * might be too few for values like UTF_STRING and OBJECT_IDENTIFIER that share a Kotlin class
     * but have very different ASN.1 interpretations.
     */
