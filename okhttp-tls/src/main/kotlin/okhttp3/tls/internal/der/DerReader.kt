...
internal class DerReader(source: Source) {
                ...
                (length0 and 0b1000_0000) == 0b1000_0000 -> {
                    ...
                    if (lengthBytes > 8) {
                        throw ProtocolException(
                            "length encoded with more than 8 bytes is not supported"
                        )
                    }
    ...
    /**
     * Execute [block] with a new namespace for type hints. Type hints from the enclosing type are
     * no longer usable by the current type's members.
     */
    ...
    companion object {
        /**
         * A synthetic value that indicates there's no more bytes. Values with equivalent data may
         * also show up in ASN.1 streams to also indicate the end of SEQUENCE, SET or other
         * constructed value.
         */
