...
internal data class BasicDerAdapter<T>(
     ...
     * ```
     *
     * You may also specify a tag class like [DerHeader.TAG_CLASS_APPLICATION]. The default tag
     * class is [DerHeader.TAG_CLASS_CONTEXT_SPECIFIC].
     *
     * ```
     * Point ::= SEQUENCE {
    ...
    fun optional(defaultValue: T? = null): BasicDerAdapter<T> =
        copy(isOptional = true, defaultValue = defaultValue)

    /**
     * Returns a copy of this adapter that sets the encoded or decoded value as the type hint for
     * the other adapters on this SEQUENCE to interrogate.
     */
