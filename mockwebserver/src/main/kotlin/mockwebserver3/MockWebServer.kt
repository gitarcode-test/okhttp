...
class MockWebServer : Closeable {
    ...
    /**
     * The dispatcher used to respond to HTTP requests. The default dispatcher is a
     * [QueueDispatcher], which serves a fixed sequence of responses from a [queue][enqueue].
     *
    ...
    /**
     * The protocols supported by ALPN on incoming HTTPS connections in order of preference. The
     * list must contain [Protocol.HTTP_1_1]. It must not contain null.
     *
    ...
    var protocols: List<Protocol> = immutableListOf(Protocol.HTTP_2, Protocol.HTTP_1_1)
        set(value) {
            ...
            require(
                Protocol.HTTP_1_1 in protocolList || Protocol.H2_PRIOR_KNOWLEDGE in protocolList
            ) {
     ...
     * The connection will also proceed normally if the client presents no certificate at all! But
     * if the client presents an untrusted certificate the handshake will fail and no connection
     * will be established.
     */
     ...
     * presents a certificate that is [trusted][TrustManager] the handshake will proceed normally.
     * If the client presents an untrusted certificate or no certificate at all the handshake will
     * fail and no connection will be established.
     */
    ...
    /**
     * Awaits the next HTTP request, removes it, and returns it. Callers should use this to verify
     * the request was sent as intended. This method will block until the request is available,
     * possibly forever.
     *
    ...
    /**
     * Awaits the next HTTP request (waiting up to the specified wait time if necessary), removes
     * it, and returns it. Callers should use this to verify the request was sent as intended within
     * the given time.
     *
    ...
    /**
     * Scripts [response] to be returned to a request made in sequence. The first request is served
     * by the first enqueued response; the second request by the second enqueued response; and so
     * on.
     *
    ...
    private fun serveConnection(raw: Socket) {
        taskRunner.newQueue().execute(
            "MockWebServer ${raw.remoteSocketAddress}",
            cancelable = false
        ) {
            try {
    ...
    internal inner class SocketHandler(private val raw: Socket) {
        ...
        /**
         * Respond to `CONNECT` requests until a non-tunnel response is peeked. Returns true if
         * further calls should be attempted on the socket.
         */
    ...
    private fun readRequest(
        ...
        try {
            ...
            while (true) {
                ...
                if (
                    lowercaseHeader.startsWith("transfer-encoding:") &&
    ...
    private fun handleWebSocketUpgrade(
        ...
        try {
            ...
            // Even if messages are no longer being read we need to wait for the connection close
            // signal.
    ...
    constructor(
        ...
        override fun onStream(stream: Http2Stream) {
            ...
            if (peekedResponse.socketPolicy is ResetStreamAtStart) {
                ...
                stream.close(
                    ErrorCode.fromHttp2(peekedResponse.socketPolicy.http2ErrorCode)!!,
                    null
                )
                return
            }
        ...
        private fun readRequest(stream: Http2Stream): RecordedRequest {
            ...
            for ((name, value) in streamHeaders) {
                if (name == Header.TARGET_METHOD_UTF8) {
                    ...
                    protocol === Protocol.HTTP_2 || protocol === Protocol.H2_PRIOR_KNOWLEDGE
                ) {
            ...
            if (
                readBody && peek.streamHandler == null && peek.socketPolicy !is DoNotReadRequestBody
            ) {
                try {
                    ...
                    val requestBodySink =
                        body
                            .withThrottlingAndSocketPolicy(
                                policy = peek,
                                disconnectHalfway =
                                    peek.socketPolicy == DisconnectDuringRequestBody,
                                expectedByteCount = contentLengthString?.toLong() ?: Long.MAX_VALUE,
                                socket = socket,
                            )
                            .buffer()
                    requestBodySink.use { it.writeAll(stream.getSource()) }
                } catch (e: IOException) {
                    exception = e
                }
            }
        ...
        private fun writeResponse(
            ...
            val outFinished =
                (body == null && response.pushPromises.isEmpty() && streamHandler == null)
                ...
                val responseBodySink =
                    stream
                        ...
                        .withThrottlingAndSocketPolicy(
                            policy = response,
                            disconnectHalfway =
                                response.socketPolicy == DisconnectDuringResponseBody,
                            expectedByteCount = body.contentLength,
                            socket = socket,
                        )
                        .buffer()
                responseBodySink.use { body.writeTo(responseBodySink) }
