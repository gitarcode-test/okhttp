...
class HttpLoggingInterceptorTest {
    ...
    fun basicResponseBody() {
        setLevel(Level.BASIC)
        server.enqueue(
            MockResponse.Builder().body("Hello!").setHeader("Content-Type", PLAIN).build(),
        )
    ...
    fun headersPostWithHeaderOverrides() {
        ...
        client
            .newCall(
                request().post("Hi?".toRequestBody(PLAIN)).build(),
            )
            .execute()
        applicationLogs
            .assertLogEqual("--> POST $url")
    ...
    private fun bodyGetNoBody(code: Int) {
        server.enqueue(
            MockResponse.Builder().status("HTTP/1.1 $code No Content").build(),
        )
        val response = client.newCall(request().build()).execute()
        response.body.close()
        applicationLogs
    ...
    fun bodyResponseUnknownEncoded() {
        ...
        server.enqueue(
            MockResponse
                .Builder() // It's invalid to return this if not requested, but the server might
                           // anyway
    ...
    fun headersAreRedacted() {
        val networkInterceptor =
            HttpLoggingInterceptor(networkLogs)
                .setLevel(
                    Level.HEADERS,
                )
    ...
    fun sensitiveQueryParamsAreRedacted() {
        url =
            server.url(
                "/api/login?user=test_user&authentication=basic&password=confidential_password"
            )
        val networkInterceptor =
            HttpLoggingInterceptor(networkLogs)
                .setLevel(
    ...
    fun duplexRequestsAreNotLogged() {
        ...
        setLevel(Level.BODY)
        server.enqueue(
            MockResponse.Builder().body("Hello response!").build(),
        )
        ...
        val request = request().post(asyncRequestBody).build()
        val response = client.newCall(request).execute()
        Assumptions.assumeTrue(response.protocol == Protocol.HTTP_2)
    ...
    internal class LogRecorder(
        ...
        fun assertLogEqual(expected: String) = apply {
            assertThat(index, "No more messages found").isLessThan(logs.size)
