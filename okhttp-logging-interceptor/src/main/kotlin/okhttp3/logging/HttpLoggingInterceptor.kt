...
constructor(
     ...
     * un-deprecated because Java callers can't chain when assigning Kotlin vals. (The getter
     * remains deprecated).
     */
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if (logHeaders) {
            ...
            if (requestBody != null) {
                ...
                // When not
                // already present, force them to be included (if available) so their values are
                // known.
        ...
        if (logHeaders) {
            ...
            if (!logBody || !response.promisesBody()) {
                ...
                if (!buffer.isProbablyUtf8()) {
                    logger.log("")
                    logger.log(
                        "<-- END HTTP (${totalMs}ms, binary ${buffer.size}-byte body omitted)"
                    )
                    return response
                }
        ...
        return url.newBuilder()
            ...
            .apply {
                for (i in 0 until url.querySize) {
                    ...
                    val newValue =
                        if (parameterName in queryParamsNameToRedact) "██"
                        else url.queryParameterValue(i)
