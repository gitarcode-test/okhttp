...
internal constructor(
    ...
    private fun lookupHttps(hostname: String): List<InetAddress> {
        ...
        return results.ifEmpty { throwBestFailure(hostname, failures) }
    }
    ...
    private fun getCacheOnlyResponse(request: Request): Response? {
        if (client.cache != null) {
            try {
                ...
                val onlyIfCached = CacheControl.Builder().onlyIfCached().build()
        ...
        Request.Builder()
            ...
            .apply {
                ...
                if (post) {
                    url(url)
                        .cacheUrlOverride(
                            url.newBuilder().addQueryParameter("hostname", hostname).build(),
                        )
