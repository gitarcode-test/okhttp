...
class OkHttpClientTestRule : BeforeEachCallback, AfterEachCallback {
                ...
                override fun publish(record: LogRecord) {
                    val recorded =
                        when (record.loggerName) {
                            ...
                            "javax.net.ssl" ->
                                recordSslDebug && !sslExcludeFilter.matches(record.message)
                            else -> false
                        }
     ...
     *
     * The returned client installs a default event listener that gathers debug information. This
     * will be logged if the test fails.
     *
    ...
    private fun loomThreadFactory(): ThreadFactory {
        ...
        return Class.forName("java.lang.Thread\$Builder").getMethod("factory").invoke(ofVirtual)
            as ThreadFactory
    }
    ...
    private fun ensureAllTaskQueuesIdle() {
        ...
        for (queue in TaskRunner.INSTANCE.activeQueues()) {
            ...
            if (!queue.idleLatch().await(waitTime, TimeUnit.NANOSECONDS)) {
                TaskRunner.INSTANCE.lock.withLock { TaskRunner.INSTANCE.cancelAll() }
    ...
    override fun afterEach(context: ExtensionContext) {
        ...
        if (uncaughtException != null) {
            throw failure +
                AssertionError("uncaught exception thrown during test", uncaughtException)
        }
    ...
    companion object {
        /**
         * A network that resolves only one IP address per host. Use this when testing route
         * selection fallbacks to prevent the host machine's various IP addresses from interfering.
         */
